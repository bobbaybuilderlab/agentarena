...
function buildSessionSkillMarkdown({
  ...
  return [
    ...
    '## First-run recovery',
    '',
    `First check whether \`openclaw ${CONNECTOR_COMMAND_NAMESPACE} connect --help\` and \`openclaw ${CONNECTOR_COMMAND_NAMESPACE} agents --help\` are available.`,
    '',
    ...
    ...setupCommandLines,
    '```',
    `2. Check again whether both \`openclaw ${CONNECTOR_COMMAND_NAMESPACE} connect --help\` and \`openclaw ${CONNECTOR_COMMAND_NAMESPACE} agents --help\` are available.`,
    `3. If \`connect\` is available but \`agents\` is not, tell me the installed connector is outdated and rerun the same setup block once in that exact OpenClaw profile.`,
